<div class="p-6 bg-zinc-900 rounded-xl shadow-2xl space-y-6 border border-zinc-700">
    <header class="mb-4">
        ...
        <p class="text-lg text-gray-400 mt-1">
            Track your assigned games and input player stats in real-time.
        </p>
    </header>
    ...
    @else
        <ul class="space-y-4">
            @foreach ($assignedGames as $game)
                <li class="p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center 
                    ...
                    <div class="mb-3 md:mb-0">
                        <p class="text-xl font-bold text-zinc-100">
                            <span class="text-red-400">{{ $game->homeTeam->name }}</span> vs <span class="text-red-400">{{ $game->awayTeam->name }}</span>
                        </p>
                        <p class="text-sm text-zinc-400 mt-1">
                            Time: <span class="font-medium text-zinc-300">{{ $game->date->format('M d, Y') }} at {{ $game->date->format('H:i') }}</span>
                        </p>
                    </div>
